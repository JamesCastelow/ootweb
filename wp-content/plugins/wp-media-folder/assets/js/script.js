        ...
        var methods = {
            init: function (o) {
                ...
                if(wpmflang.wpmf_role == 'administrator'){
                    if (options.showroot != '') {
                        $this.html('<ul class="jaofiletree"><li data-id="0" class="directory collapsed selected" data-group="'+ wpmflang.wpmf_curent_userid +'"><a class="title-folder title-root" href="#" data-id="0" data-file="' + options.root + '" data-type="dir">' + options.showroot + '</a></li></ul>');
                    }
                    openfolder(options.root);
                }else{
                    if(wpmflang.wpmf_active_media == 1 && wpmflang.term_root_id){
                        $this.html('<ul class="jaofiletree"><li  data-id="'+ wpmflang.term_root_id +'"  class="directory collapsed selected"><a class="title-folder title-root" href="#" data-id="'+ wpmflang.term_root_id +'" data-file="/' + wpmflang.term_root_username + '/" data-type="dir">' + wpmflang.term_root_username + '</a></li></ul>');
...
// --------- end folder tree --------------------------------
                                ...
                                if(typeof(response.term_id)!=='undefined'){
                                    ...
                                    $('.wpmf-attachments-browser').append('<li class="wpmf-attachment" data-parent_id="'+response.parent+'" data-id="'+response.term_id+'">'+
                                                        '<div class="wpmf-attachment-preview">'+
                                                            ...
                                                            //'<img src="'+wpmflang.wpmf_images_path+'/xxx-folder.png" class="icon" draggable="false">'+
                                                            '<div class="filename">'+
        ...
        initAttachments = function(){ 
            ...
            if(wpmfcheckMimetype === true) return;
        ...
        changeCategory = function(){
            ...
            if(wpmflang.wpmf_active_media == 1 && wpmflang.wpmf_role !='administrator' && page !='table' && wpmflang.term_root_id){
                if(selectedId == 0) {
                    $('select.wpmf-categories option[data-id="'+ wpmflang.term_root_id +'"]').prop('selected',true).change();
                    wpmfattachmentsbrowser.html(null);
                }
            }
            //folder tree
                ...
                if(page == 'table'){
                    ...
                    if($('[id^="__wp-uploader-id-"]:visible .media-frame.hide-menu').length === 0){
                        ...
                        if($('[id^="__wp-uploader-id-"]:visible .attachments-browser .wpmf-attachments-browser').find('#jao').length === 0){
                            $('.attachments-browser .wpmf-attachments-browser').before(menu_left); 
        ...
        bindAttachmentEvent = function(){
            ...
            $('.wpmf-attachment').unbind('click').bind('click',function(e){
                if($(this).hasClass('wpmfnoclick')) return;
        ...
        if ( wp.media && $('body.upload-php table.media').length===0 ) {
                    ...
                    createFilters: function() {
                        ...
                        _.each( wpmflang.wpmf_categories_order || [], function( key ) {
                         ...
                         term =  wpmflang.wpmf_categories[key];    
                            ...
                            if(typeof term.depth == 'undefined') { term.depth = 0; }
                            filters[ ij ] = {
                    ...
                    refresh:function(){
                        ...
                        if(wpmflang.wpmf_pagenow != 'upload.php'){
                            ...
                            if(typeof wpmflang.duplicate != 'undefined' && wpmflang.duplicate == 1 ){
                                if($('[id^="__wp-uploader-id-"]:visible .attachment-info .wpmf_wrap_duplicate').length === 0){
                                    $('[id^="__wp-uploader-id-"]:visible .attachment-info').append($('.wpmf_wrap_duplicate'));
                                }
                            }
            ...
            wp.media.view.Modal = wp.media.view.Modal.extend({
                open : function(){
                    ...
                    if(wpmflang.wpmf_pagenow == 'upload.php'){
                        ...
                        if(typeof wpmflang.duplicate != 'undefined' && wpmflang.duplicate == 1 ){
                            $('.details').append($('.wpmf_wrap_duplicate'));
                        }
                    }
            ...
            if(typeof wpmflang.wpmf_categories == "undefined") return;
